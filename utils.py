...
def set_dataset4(txt_file, bs, data_set, num_workers, preprocess, joint_text, mtl, test):
    if mtl == 0:
        is_aigc2023 = True
    else:
        is_aigc2023 = False
...
def convert_norm_score(ori_obj_score):
    b = ori_obj_score.shape[0]
    max_score = torch.max(ori_obj_score)
    min_score = torch.min(ori_obj_score)
    norm_score = (ori_obj_score - min_score) / max_score
    return norm_score
